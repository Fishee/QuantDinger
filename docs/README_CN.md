...
| **生产级基础设施** | **PostgreSQL 18** + **Redis 7**、连接池、后台 Worker（挂单、组合监控、反思任务）、幂等 schema 引导、GHCR 多架构镜像（amd64/arm64）。 |
...
- **平台** — Docker Compose + GHCR 镜像、PostgreSQL 18、Redis 7、OAuth、多用户 RBAC、积分/会员/USDT 计费开关、11 语种 Web UI 与多语言文档。

## 架构

**设计原则：** **行情采集**、**策略/回测计算**、**订单执行** 分层解耦——除非你显式上线策略，否则研究路径不会与真实资金共用代码。

**栈结构：** Nginx 提供预构建 Vue SPA（`ghcr.io/brokermr810/quantdinger-frontend`）；**Flask + Gunicorn** API 承载策略、AI、计费与 Agent 服务；**PostgreSQL 18** 为系统记录库；**Redis 7** 支撑缓存与 Worker 协调。交易所、经纪商、LLM、支付通过 env 驱动适配器接入——换供应商无需 fork 核心代码。
...
绝大多数应用运行时行为由 **`backend_api_python/.env`** 控制（管理员、LLM、工作进程、计费、券商等）。

**仓库根目录**下的 `.env` 用于 Docker Compose 编排：暴露端口、镜像前缀 `IMAGE_PREFIX`、镜像 tag、Postgres 镜像版本、Postgres 数据目录挂载、`PGDATA`，以及 Compose 自动生成的 `DATABASE_URL`。建议把边界记清楚：

| 文件 | 负责内容 |
|------|----------|
| `backend_api_python/.env` 或 `backend.env` | Flask API 的业务运行配置。 |
| 仓库根目录 `.env` | Docker Compose 变量替换，例如端口、镜像、Postgres/Redis 连接与宿主机路径。 |
...
全新安装不用处理，Compose 会创建自己的 Postgres 数据卷。

已有生产库时，如果数据量可控，优先使用 `pg_dump` / `pg_restore`。如果数据太大，只能物理挂载旧数据目录，必须同时满足：
...
Compose 后端默认使用 **Gunicorn** 运行 API，并把 `nofile` 设置为 `65535`。这是推荐的生产部署路径。不要在公网服务器上直接用 `python run.py` 跑后端；它更适合开发环境，而且经常继承 `1024` 这种较低的文件描述符上限。
...
期望结果：`ulimit -n` 足够高，进程命令里能看到 Gunicorn。如果通过 1Panel 或其他自定义运行时部署，需要在对应运行时里设置等价的 `nofile` 上限，并确认反向代理指向当前后端端口。
...
| `Too many open files` 或 `ulimit -n` 仍是 `1024` | 使用项目自带 Compose 后端，或在自定义运行时设置 `nofile` 到 `65535` 左右，并重建容器；只重启通常不会改变旧容器限制。 |
...
用于**自定义端口**、**拉取基础镜像过慢**、固定镜像 tag，或物理挂载已有 Postgres 数据目录。在**与 `docker-compose.yml` 同级**的目录创建 `.env`：
